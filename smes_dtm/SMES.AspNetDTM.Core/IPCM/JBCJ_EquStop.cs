...
namespace SMES.AspNetDTM.Core.IPCM
{
    public class JBCJ_EquStop : ServiceBase, IJBCJ_EquStop
    {
        ...
        public DataTable JBCJ_JT_Month(string CODE, string DATE)
        {
            ...
            if (CODE.Equals("team"))
            {
                sql = @"DECLARE @DATE VARCHAR(20);
                        SET @DATE='{1}'  --2017-08
                        SELECT T.NAME MACH_ID, LEFT(C.PRODATE,7) YM,SUM(CONVERT(FLOAT,A.STOPTIMES)) STOPCS
						,round(sum(convert(float,a.STOPTIME))/60,2) STOPTIME
                        from [ITG].[jb_transEntryFeedBackInfoOrder_TJTJ_INFO]  a
                        left join itg.Log b on b.id=a.logid
                        left join [ITG].[jb_transEntryFeedBackInfoOrder_GDSCFK_INFO] c on c.logid=b.id
						LEFT JOIN DPS.TEAM T ON C.TEAM_ID=T.CTRL
                        WHERE LEFT(CONVERT(VARCHAR,C.PRODATE,23),7)>=LEFT(CONVERT(VARCHAR,DATEADD(MONTH,-11,GETDATE()),23),7)
                        and left(c.PRODATE,7)=@DATE
                        GROUP BY T.NAME,T.CTRL,LEFT(C.PRODATE,7) ORDER BY T.CTRL";
            }
            else
            {
                sql = @"DECLARE @JTTYPE VARCHAR(20),@JTH VARCHAR(20),@DATE VARCHAR(20);
                        SET @JTTYPE='{0}';
                        SET @JTH=@JTTYPE+'%';
                        SET @DATE='{1}'   --'2017-08'
        ...
        public DataTable JBCJ_Team_Month(string CODE, string DATE)
        {
            string sql = @"DECLARE @DATE VARCHAR(20);
                        SET @DATE='{1}'  --2017-08
                        SELECT T.NAME, LEFT(C.PRODATE,7) YM,SUM(CONVERT(FLOAT,A.STOPTIMES)) STOPCS
						,round(sum(convert(float,a.STOPTIME))/60,2) STOPTIME
                        from [ITG].[jb_transEntryFeedBackInfoOrder_TJTJ_INFO]  a
                        left join itg.Log b on b.id=a.logid
                        left join [ITG].[jb_transEntryFeedBackInfoOrder_GDSCFK_INFO] c on c.logid=b.id
        ...
        public DataTable JBCJ_JT_Day(string CODE, string DATE)
        {
            ...
            {
                sql = @"DECLARE @DATE VARCHAR(20);
                             SET @DATE='{1}'  --2017-08-09
                            select  T.NAME MACH_ID, C.PRODATE ,SUM(CONVERT(FLOAT,A.STOPTIMES)) STOPCS, round(sum(convert(float,a.STOPTIME))/60,2) STOPTIME
                            from [ITG].[jb_transEntryFeedBackInfoOrder_TJTJ_INFO]  a
                            left join itg.Log b on b.id=a.logid
                            left join [ITG].[jb_transEntryFeedBackInfoOrder_GDSCFK_INFO] c on c.logid=b.id
							LEFT JOIN DPS.TEAM T ON C.TEAM_ID=T.CTRL
                            WHERE  convert(varchar,c.PRODATE,23)=@DATE
                            GROUP BY T.NAME,T.CTRL,PRODATE   ORDER BY T.CTRL  ";
            }
            else
            {
                sql = @" DECLARE @JTTYPE VARCHAR(20),@JTH VARCHAR(20),@DATE VARCHAR(20);
